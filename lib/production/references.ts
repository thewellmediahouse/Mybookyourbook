...
export async function buildVideoSubmitInput(
  ...
  if (input.signGetUrl) {
    ...
    const byRole = new Map(identityRows.map((row) => [row.role, row.objectKey]));
    for (const slot of PHOTO_SLOTS) {
      const key = byRole.get(slot);
      if (!key) {
        throw new Error("IDENTITY_INCOMPLETE");
      }
      imageUrls.push(await input.signGetUrl(key));
    }
    const videoKey = byRole.get("IDENTITY_VIDEO");
    if (!videoKey) {
      throw new Error("IDENTITY_INCOMPLETE");
    }
    videoUrls.push(await input.signGetUrl(videoKey));
