...
/**
 * Contact form — maps to SITE_SPEC `form` (docs/SITE_SPEC.schema.json).
 *
...
export type FormFieldType = 'text' | 'email' | 'tel' | 'select' | 'textarea' | 'date' | 'file';
...
export interface FormField {
  ...
  halfWidth?: boolean;
  /** Optional section heading rendered before this field when it changes. */
  section?: string;
  /** Helper text under the label (e.g. file limits). */
  hint?: string;
  /** Empty / “None” option label for selects. */
  emptyOptionLabel?: string;
  /** File input `accept` attribute. */
  accept?: string;
  /** Client-side max upload size in MB (validated before submit). */
  maxSizeMb?: number;
  /** Allow selecting more than one file (file inputs only). */
  multiple?: boolean;
}
...
export const formConfig: FormConfig = {
  ...
  successMessage: 'Thank you — your message has been sent. We will respond within one business day.',
