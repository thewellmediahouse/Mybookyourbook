/**
 * Core site settings — maps to SITE_SPEC `site` (docs/SITE_SPEC.schema.json).
 */
export const siteConfig = {
  ...
  /**
   * Fallback public path when `raster.og.default` is unset.
   * Prefer a branded photo in `src/assets/` → `raster.og.default` (1200×630 via getImage).
   * Do not ship a decorative SVG as the long-term sitewide share image.
   */
