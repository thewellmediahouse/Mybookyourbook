...
`FILMING_AI_MODE=mock|live` controls Filming Your Commercial only. `live` calls reAPI Seedance 2.5. Missing `REAPI_API_KEY` must not silently mock. When unset, filming follows `AI_PROVIDER_MODE`.

`ENHANCEMENT_AI_MODE=mock|live` controls Enhancing Your Footage only. `live` calls the Topaz Video API after the filmed file is in private R2 and before branding. Missing `TOPAZ_API_KEY` must not silently mock. When unset, enhancement follows `AI_PROVIDER_MODE`. Mock enhancement keeps the filmed file (no fixture swap).
