...
- PUT signs `Content-Type`. The browser must send the same header. Allowlist: `image/png`, `image/jpeg`, `image/webp` only. No SVG. Max 5 MB for logos.
...
- Same signed PUT / binding PUT path as logos and identity. Library stills and logos: PNG/JPEG/WebP up to 8 MB. No SVG. Complete responses are `{ ok, assetId }` only — no public URL and no stored signed URL.
...
- Local `next dev` / `getPlatformProxy` do not bind Workflows. `/api/production/start` then uses `waitUntil` + mock providers so the browser is not held on filming. Credit is reserved **before** `create` / `waitUntil`. Duplicate in-flight starts are rejected. Technical failure refunds once. A Worker publish mid-job can throw `Durable Object reset because its code was updated.` That is not a customer failure: do not mark FAILED, do not email, do not refund. The same job continues after the new version starts.
...
- Locked input (official default is 720p / `duration` 5 / `size` adaptive): `resolution: "480p"`, `duration` is the integer the customer chose (10 / 15 / 20 / 30). Official Seedance 2.5 range is 4–30 or `-1` auto (fetched 2026-08-30). We never send `-1`. Older code always sent `30` even when the brief was 15s — that is fixed. `size` from the brief (`9:16` / `16:9` / `1:1`; never `auto` or `adaptive`), `generate_audio: true`, `output_format: "mp4"`, `omni_reference_task_type: "reference"` so submit is checked as a reference job (official 2026-08-30). Official optional fields used: `image_urls` (≤30), `video_urls` (≤10). Official stills: jpeg/png/webp/bmp/tiff/gif/heic/heif. Identity maps `@Image1–3` / `@Video1`. A raster brand logo (not SVG) is `@Image4` when present; campaign stills then `@Image5–10`. Without a raster logo, campaign stills stay `@Image4–9`. Empty arrays are omitted. Not sent: `audio_urls`, `image_with_roles`, `tools`, `return_last_frame`, `content_filter: false` (default safety stays on), `bitrate_mode`.
...
- Workflow still polls with `step.sleep` 15s, up to 180 attempts (45 minutes). Official task statuses are only `processing` / `completed` / `failed` ([Tasks](https://reapi.ai/docs/api/tasks), fetched 2026-08-30). Video wall-clock cap is 48 hours; most finishes in minutes. We treat only official `failed` / `error` / cancelled as failed. `queued` / `pending` / empty / HTTP 5xx / network errors stay processing so a brief envelope glitch cannot abort a finished take. `completed` aliases (`complete` / `success`) and a present `output.video_urls` HTTPS URL count as complete. HTTP 401/403 fail. HTTP 429 stays processing. Polling does not consume credits. A stored `video_provider_job_id` is reused on retry so we do not POST a second generation for the same Ad Credit. Official Workflows persist `step.do` JSON with a 1 MiB cap — the filmed file is downloaded into private R2 inside `save-source-r2` / `save-enhanced-r2`; step results store asset ids only, never video bytes. `FILMING_AI_MODE=live` (or `AI_PROVIDER_MODE=live` when filming mode is unset) calls reAPI. `FILMING_AI_MODE=mock` never calls reAPI even if `REAPI_API_KEY` is set. Live without a key does not silently mock. Customer errors stay “Live filming is not connected yet.” / “We couldn't complete filming right now.” Reference files use short-lived R2 S3 GET URLs when those secrets are set; otherwise a 1-hour HMAC GET on `/api/provider/files/{token}` streamed from the `MEDIA_BUCKET` binding (`INTERNAL_SERVICE_SECRET`). Official: no `data:` URIs. Missing signer fails immediately (`REFERENCES_UNSIGNED`) without the default 5-minute Workflow retry. Mock enhancement/branding keep the filmed file; they do not replace it with the development fixture.
...
Card previews were blank because `capBytesRange` turned a no-Range GET into a 1 MB **206**. Files with the index at the end never started. `kind: "all"` is no longer sliced. The card loop uses the filmed source, not the enhanced finished file, and the player resets at 3 seconds so the Worker is not asked for the whole take.
...
- `BETTER_AUTH_URL` and `NEXT_PUBLIC_APP_URL` are `https://production30.thewellmedia.com`. Better Auth `trustedOrigins` includes that host and workers.dev. GitHub is now connected to Worker `cineyou` Builds. First build `e5e68016-3fea-4731-9924-ed546db162c8` (commit `0cef57e`) failed: defaults were `npm run build` then `npx wrangler deploy`. `next build` is not OpenNext, so deploy died with “Could not find compiled Open Next config”. Official [configuration](https://developers.cloudflare.com/workers/ci-cd/builds/configuration/): set **Build command** `npm run cf:build` and **Deploy command** `npm run cf:deploy:ci`, then Retry. Do not leave deploy as `npx wrangler deploy` (rebuilds the branding container and re-PUTs queues). Wrangler OAuth still cannot PATCH `/builds/triggers` (error 10000). Build vars: `NEXT_PUBLIC_APP_URL=https://production30.thewellmedia.com`, `NEXT_PUBLIC_APP_NAME=Production30`. Do not Git-connect this repo as Pages.
...
### 2026-08-30 — Customer Ad Credit follows a live filming charge

Official tasks docs: `completed` settles `usage.credits`; `failed` is refunded by reAPI; `processing` is not settled yet. If we submitted a live task id and the task is not `failed`, we do not return the customer Ad Credit. If an earlier false failure already refunded them, saving the filmed file takes that credit back (`filming-charged:` idempotency). Mock task ids still refund. Customer download copy is “Download”, not 1080p — source filming is 480p.

### 2026-08-30 — First Workers Builds run used Next.js defaults

GitHub is connected to Worker `cineyou`. Build `e5e68016-3fea-4731-9924-ed546db162c8` on `0cef57e` failed after a successful `npm run build` (`next build`). Deploy `npx wrangler deploy` then called OpenNext and died: “Could not find compiled Open Next config”. Official [Builds configuration](https://developers.cloudflare.com/workers/ci-cd/builds/configuration/): set Build `npm run cf:build` and Deploy `npm run cf:deploy:ci`, then Retry. Leave `npm run build` as `next build` (OpenNext calls it). Do not deploy with bare `npx wrangler deploy` (container rebuild + queue consumer PUT). Wrangler OAuth cannot PATCH `/builds/triggers` (10000).

### 2026-08-30 — Studio previews need HTTP Range

Official R2 (fetched 2026-08-30): [Workers API usage](https://developers.cloudflare.com/r2/api/workers/workers-api-usage/) `bucket.get(key, { range })` / [Ranged reads](https://developers.cloudflare.com/r2/api/workers/workers-api-reference/#ranged-reads). Studio cards play `/api/assets/{id}` in a muted looping `<video>`. Browsers send `Range: bytes=…` for that. A 200 of the whole file with no `Accept-Ranges` leaves a blank card (Safari especially). `streamPrivateAsset` now heads the object, answers a single byte range with **206** + `Content-Range`, streams `object.body` (no full `arrayBuffer` for playback), and still uses `Content-Disposition: inline` unless `?download=1`. Studio lists a playable file from the newest `finalAssetId`, else a `COMPLETE` job’s filmed source.

### 2026-08-30 — No filmed writing (480p cannot spell)

Live 480p takes turn small letters and small call-to-action graphics into unreadable marks. The old “important readable written text” rule left room for decorative / tiny type. Filming prompts now ban all written characters, including tiny CTAs, buttons, lower thirds, and background shopfront type. Concept generation is told the same. Scene lines that ask for signs or on-screen words are stripped before submit. Produce rebuilds the prompt so already-approved concepts cannot film that writing. Branding and exact copy stay post. No extra reAPI fields.
