...
- `BETTER_AUTH_URL` and `NEXT_PUBLIC_APP_URL` are `https://production30.thewellmedia.com`. Better Auth `trustedOrigins` includes that host and workers.dev. Live Worker `cineyou` version `7a54ecbd-c6d0-47b0-8ab2-21ae8d63874f` (tag `github-7e33322`) was promoted from this laptop. GitHub Actions still only typechecks: the repository `CLOUDFLARE_API_TOKEN` cannot publish a Worker version. Workers Builds has zero builds — the Cloudflare GitHub App is not connected to Worker `cineyou`. Official [Builds](https://developers.cloudflare.com/workers/ci-cd/builds/) + [Builds API](https://developers.cloudflare.com/workers/ci-cd/builds/api-reference/) (fetched 2026-08-30): connect is dashboard-only the first time (Worker → Settings → Builds → Connect → GitHub → `thewellmediahouse/Mybookyourbook`). After that, pushes to `main` build and deploy. Wrangler OAuth cannot call `/builds/*` (error 10000; needs a user-scoped token with Workers Builds Configuration Edit). Do not Git-connect this repo as Pages. When connecting Builds, set build `npm run cf:build` and deploy `npm run cf:deploy:ci` (versions upload + promote; `--keep-vars`; no container rebuild; no queue consumer PUT). Build vars: `NEXT_PUBLIC_APP_URL=https://production30.thewellmedia.com`, `NEXT_PUBLIC_APP_NAME=Production30`.
