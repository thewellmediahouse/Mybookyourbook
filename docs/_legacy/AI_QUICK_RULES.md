...
- **Lockfile:** Keep `package-lock.json` compatible with Pages **npm 10.9.x** `npm ci` (Pages keeps npm 10 even when `.nvmrc` sets Node 24). Regenerate under npm 10 after dependency changes; see Cloudflare section in `AI_TEMPLATE_RULES.md`.
...
5. Put rasters in `src/assets/` + `raster.ts` (use `OptimizedImage` / `BrandLogo`); keep SVG/favicon/video in `public/`.
