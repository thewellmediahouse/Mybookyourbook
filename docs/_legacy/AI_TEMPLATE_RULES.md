...
src/
  assets/
    raster.ts       # ImageMetadata registry for OptimizedImage / BrandLogo
    brand/          # raster logos (webp/png)
    images/         # hero, service, team, OG, and general imagery
  ...
  components/
    ...
    ui/             # Button, Card, Badge, BrandLogo, OptimizedImage, Container, etc.
...
* `Card`
* `Badge`
* `BrandLogo` — retina logo via `astro:assets` `Picture` (raster `ImageMetadata`)
* `OptimizedImage` — responsive WebP images with optional mobile `<source>`
...
* Body scale
* Small text scale
* Display font and body font — self-host via `@fontsource/*` (default: Inter 400/700 + Playfair Display 700). Avoid Google Fonts CDN; preload the woff2 files in `BaseLayout`.
...
Prefer the **Astro image pipeline** for all raster photography and brand marks (webp, jpg, png):

```txt
src/assets/
  raster.ts          # import registry — export ImageMetadata for components/config
  brand/             # logos (webp/png), not SVG
  images/            # hero, services, portfolio, testimonials, OG, etc.
```

Use platform helpers:

* `OptimizedImage` — responsive WebP via `astro:assets` (optional mobile `<source>`)
* `BrandLogo` — sized logo `Picture` with retina widths
* `src/utils/media.ts` — `resolveMediaSrc` / `isImageMetadata` when config may be metadata or a public string
* `getImage()` in `SEO.astro` when `raster.og.default` is a raster import

Keep in **`public/` only**:

* SVG placeholders and icons
* Favicon / apple-touch / webmanifest icons
* Video files (webm/mp4) and other non-image static assets

```txt
public/
  favicon.svg
  images/            # SVG placeholders only when needed
  gallery/           # videos / SVG stubs — not large rasters
```

Do **not** ship large unoptimized jpg/png/webp from `public/` — move them under `src/assets/`, register in `raster.ts`, and render with `OptimizedImage` / `BrandLogo`.

Config may still use string paths for SVG placeholders during early builds (`/images/hero.svg`). When final rasters arrive, switch config/components to `raster.*` imports.

Requirements:

* Responsive `widths` + `sizes` (and `mobileWidths` on large heroes when needed)
* Descriptive alt text
* Lazy-load non-critical images; `fetchpriority="high"` only on LCP candidates
* Avoid oversized source files before the pipeline runs
...
* Portfolio items
* Video poster/thumbnail rasters
* Open Graph default image (prefer 1200×630 jpg/webp in `src/assets`)

Fallback rule: if final images do not exist, use SVG placeholders in `public/images/` with descriptive alt text. Replace with `src/assets` + `raster.ts` when client assets arrive.
...
### Node / npm and lockfiles

Pages **v3** defaults to Node 22 + **npm 10.9.2**. Setting `.nvmrc` (template default: `24`) upgrades Node only — **npm stays 10.9.2** on Pages (`NPM_VERSION` is not available on v3). Local Node 24 typically ships npm 11+.

Lockfile rule for Cloudflare:

* Pages runs `npm ci`. `package.json` and `package-lock.json` must stay in sync for **npm 10.9.x**, not only the local npm major.
* Prefer regenerating the lockfile with npm 10 after dependency changes, e.g. `nvm use 22` (npm 10) then `npm install`, or `npx npm@10.9.2 install`.
* npm 11 lockfiles can omit optional `@emnapi/core` / `@emnapi/runtime` entries that npm 10 `ci` requires — if CI fails with those missing, materialize them in the lockfile (or regenerate under npm 10) before pushing.
* Verify with `npx npm@10.9.2 ci` (or equivalent) before relying on a Pages deploy.

For Workers/assets deploys via Wrangler (optional):

* `wrangler` is a `devDependency`; rename `wrangler.jsonc` `"name"` per site
* `npm run deploy` runs `astro build` then `wrangler deploy`
* Prefer Cloudflare Pages Git builds for most sites; use Wrangler when you need CLI deploy
